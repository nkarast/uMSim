/**
 *  Source file for \sa UMDetectorConstruction
 *
...
/// user defined function to fined the density of a gas mixture
...
void UMDetectorConstruction::DefineMaterials()
{
    ...
    density = 3.483539*g/cm3;
    ResStripMat = new G4Material(name ="ResStripMat", density, ncomponents=2);
    ResStripMat->AddMaterial(Epoxy, fractionmass=78.57*perCent); /// epoxy is glue
    ResStripMat->AddMaterial(Graphite, fractionmass=21.43*perCent); /// and graphite
...
G4VPhysicalVolume* UMDetectorConstruction::ConstructDetector()
{
    ...
    G4bool checkOverlaps = false;  /// this makes the software slower but checks for overlaps during debugging phase
