...
particleGun(0)
{
    ...
    // detector construction related for PARALLEL BEAM
    dummy1 = config.pcb_halfY;
    dummy2 = config.pcb_halfZ;    
...
void UMPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
	...
	particleGun_Xdistance = 0.0*um;
        particleGun_Ydistance = 2300000.*um;
        particleGun_Z_distance = 0.0*um;
     ...
     // ===>> to calculate the angles we subtract the half_width of the detector
       G4double bestDistance = particleGun_Ydistance-dummy1;
      //G4cout << "[INFO] PrimaryGenerator : best distance (gun_distance - detector_half_thickness = " << bestDistance << G4endl;
                                                                                                    
       G4double max_height = dummy2; //sqrt((detector_half_width*detector_half_width)+(detector_half_width*detector_half_width));
    ...
    G4double ux = cos(phi) * sqrt(1-cosrand*cosrand);
                   
                                                                                                                                                                                      // set the momentum of y-axis towards the negative direction
